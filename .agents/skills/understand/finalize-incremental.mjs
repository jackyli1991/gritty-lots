      ...
      description:
        typeof layer?.description === 'string'
          ...
          : index === 0
            ? 'Project files'
            : '',
      nodeIds: refs.map((ref) => resolveNodeRef(ref, nodeIds, pathIndex)).filter(Boolean),
    };
  });
}
        ...
        description:
          typeof step?.description === 'string'
            ...
            : typeof step?.whyItMatters === 'string'
              ...
              : '',
        nodeIds: [
          ...new Set(refs.map((ref) => resolveNodeRef(ref, nodeIds, pathIndex)).filter(Boolean)),
        ],
      };
...
function isGeneratedOnly(plan) {
  return (
    plan.generatedArtifactFiles.length > 0 &&
...
async function main() {
  ...
  if (plan.action === 'SKIP') {
    ...
    if (
      ...
      !Array.isArray(previousGraph.edges)
    ) {
  ...
  if (plan.action !== 'SKIP') {
    ...
    if (!symbolReport.ok)
      throw new Error('Unresolved incremental symbol loss; baseline not advanced');
