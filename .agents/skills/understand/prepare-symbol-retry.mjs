...
async function main() {
  if (process.argv.length !== 3)
    throw new Error('Usage: node prepare-symbol-retry.mjs <projectRoot>');
  ...
  const batching = spawnSync(
    process.execPath,
    [
      join(skillDir, 'compute-batches.mjs'),
      projectRoot,
  ...
  if (batching.status !== 0)
    throw new Error(`Retry batching failed: ${batching.error ?? batching.status}`);
  ...
  for (const batch of batches) {
    if (!Number.isInteger(batch.batchIndex) || batch.batchIndex < 1)
      throw new Error('Invalid retry batch index');
    const files = new Set(batch.files.map((file) => file.path));
    batch.previousSymbols = baseline.files
      .filter((file) => files.has(file.filePath))
      .flatMap((file) =>
        file.nodes.filter(symbolKind).map((node) => {
          ...
          const owners = file.nodes
            .filter((parent) => parent.type === 'class' && parents.has(parent.id))
  ...
  if (
    candidates.baseCommit !== plan.baseCommit ||
    candidates.headCommit !== plan.headCommit ||
    !Array.isArray(candidates.edges)
  )
    throw new Error('Current edge candidates do not match the incremental plan; rerun merge');
  const removedIds = new Set(
    assembled.nodes.filter((node) => paths.has(normalizePath(node.filePath))).map((node) => node.id)
  );
  const retainedIds = new Set(
    assembled.nodes.filter((node) => !removedIds.has(node.id)).map((node) => node.id)
  );
  const currentNodes = new Map(assembled.nodes.map((node) => [node.id, node]));
  const previousIds = new Set(
    baseline.files
      .filter((file) => paths.has(file.filePath))
      .flatMap((file) => file.nodes.map((node) => node.id))
  );
  const targetsAffectedFile = (id) =>
    removedIds.has(id) ||
    previousIds.has(id) ||
    (typeof id === 'string' && [...paths].some((path) => id.includes(`:${path}:`)));
  const sourceRemap = new Map(
    report.files.flatMap((file) => file.replacements).map(({ oldId, newId }) => [oldId, newId])
  );
  const inboundEdgeCandidates = [...assembled.edges, ...candidates.edges]
    .map((edge) => ({
      ...edge,
      source: currentNodes.has(edge.source)
        ? edge.source
        : (sourceRemap.get(edge.source) ?? edge.source),
    }))
    .filter((edge) => retainedIds.has(edge.source) && targetsAffectedFile(edge.target));
  ...
  atomicWriteJson(retryPath, {
    ...
    currentFiles: [...contextPaths].map((filePath) => {
      ...
      return {
        ...
        nodes,
        edges: assembled.edges.filter(
          (edge) => edge.type === 'contains' && ids.has(edge.source) && ids.has(edge.target)
        ),
      };
    }),
  });
  ...
  process.stdout.write(
    `Prepared symbol retry 1/1 for ${paths.size} file(s); dispatch batches from ${retryPath}\n`
  );
}
