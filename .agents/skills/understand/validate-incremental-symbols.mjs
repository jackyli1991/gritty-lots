...
function validScope(scope) {
  return (
    scope &&
    (['file', 'unknown'].includes(scope.kind) ||
...
function validSupplement(evidence) {
  ...
  const validEntry = (item) =>
    item &&
    [null, 'callable', 'class'].includes(item.kind) &&
    validScope(item.scope) &&
    (item.name === null || typeof item.name === 'string') &&
    typeof item.reason === 'string' &&
    validRange(item.lineRange) &&
    (item.nameSuffix === undefined || typeof item.nameSuffix === 'string');
  const validDeclaration = (item) =>
    item && typeof item.name === 'string' && validScope(item.scope) && validRange(item.lineRange);
  return (
    supplement?.version === 2 &&
    Array.isArray(supplement.effects) &&
    supplement.effects.every(validEntry) &&
    Array.isArray(supplement.functions) &&
    supplement.functions.every(validDeclaration) &&
    Array.isArray(supplement.classes) &&
    supplement.classes.every(validDeclaration) &&
    supplement.coverage?.profile === 'structural-declarations-v1' &&
    Array.isArray(supplement.coverage.gaps) &&
    supplement.coverage.gaps.every(validEntry)
  );
}
...
function compatibleEvidence(records, symbol) {
  return records.filter((item) => {
    ...
    if (item.scope.kind !== 'unknown' && qualifiedName(owner) !== qualifiedName(symbol.owner))
      return false;
...
function sourceSymbols(evidence) {
  ...
  for (const fn of evidence.symbolEvidence.functions) {
    if (fn.scope.kind !== 'local')
      symbols.push({
        kind: 'callable',
        owner: scopeOwner(fn.scope),
        name: fn.name,
  ...
  for (const fn of functions.filter((fn) => fn.owner === undefined)) {
    if (
      !evidence.symbolEvidence.functions.some(
        (candidate) =>
  ...
  for (const cls of classes) {
    const declaration = evidence.symbolEvidence.classes.find(
      (item) =>
        item.name === cls.name && JSON.stringify(item.lineRange) === JSON.stringify(cls.lineRange)
    );
    if (declaration?.scope.kind === 'local') continue;
    const classVerified = declaration?.scope.kind === 'file';
    symbols.push({
      kind: 'class',
      owner: classVerified ? '' : null,
      name: cls.name,
    ...
    for (const name of cls.methods) {
      ...
      const detailed = symbols.filter(
        (symbol) => symbol.kind === 'callable' && symbol.owner === cls.name && symbol.name === name
      );
      ...
      if (
        classes.filter((other) => other.name === cls.name).length === 1 &&
        count < detailed.length
      )
        continue;
      const unknownOwner = symbols.some(
        (symbol) => symbol.kind === 'callable' && symbol.name === name && symbol.owner === null
      );
      symbols.push({
        kind: 'callable',
        owner: unknownOwner || !classVerified ? null : cls.name,
        name,
        lineRange: cls.lineRange,
      });
    }
...
function ambiguousOwner(symbol, source) {
  return (
    symbol.owner &&
    source.filter((item) => item.kind === 'class' && item.name === symbol.owner).length > 1
  );
}
...
function classOwners(node, graph) {
  const parents = new Set(
    graph.edges
      .filter((edge) => edge.type === 'contains' && edge.target === node.id)
...
function hasPreservedIdentity(node, previous, current, sameRevision = false) {
  const candidate = current.nodes.find(
    (candidate) =>
      candidate.id === node.id &&
      symbolKind(candidate) === symbolKind(node) &&
      candidate.name === node.name
  );
  if (!candidate) return false;
  const owners = (item, graph) =>
    classOwners(item, graph)
      .map((owner) => JSON.stringify([owner.id, owner.name]))
      .sort();
  ...
  if (
    symbolKind(node) === 'callable' &&
    oldOwners.length === 0 &&
    (!sameRevision || JSON.stringify(node.lineRange) !== JSON.stringify(candidate.lineRange))
  )
    return false;
  return true;
}
...
function resolveSymbol(node, graph, symbols) {
  ...
  const qualified = candidates.filter(
    (symbol) => symbol.owner && names.has(qualifiedName(`${symbol.owner}.${symbol.name}`))
  );
  if (qualified.length) candidates = qualified;
  const owners = classOwners(node, graph).map((parent) => qualifiedName(parent.name));
  if (owners.length)
    candidates = candidates.filter(
      (symbol) => symbol.owner !== null && owners.includes(qualifiedName(symbol.owner))
    );
...
export function compareFileSymbols(
  previous,
  current,
  baseEvidence,
  headEvidence,
  ...
  for (let index = 0; index < oldSymbols.length; index++) {
    ...
    if (
      baseEvidence?.status === 'succeeded' &&
      headEvidence?.status === 'succeeded' &&
      !evidenceValid
    ) {
      ...
      entry.reason =
        'Old symbol cannot be mapped uniquely, or source parsing is unavailable/failed';
      ...
      oldMappings.filter((symbol) => symbol && symbolKey(symbol) === symbolKey(old)).length !== 1 ||
      ...
      const graphMatches = newMappings.filter(
        (symbol) => symbol && symbolKey(symbol) === symbolKey(old)
      );
      if (
        ambiguousOwner(old, baseSource) ||
        matches.some((symbol) => ambiguousOwner(symbol, headSource))
      ) {
      ...
      if (matches.length === 1 && graphMatches.length === 1) {
        const matchedIndex = newMappings.findIndex(
          (symbol) => symbol && symbolKey(symbol) === symbolKey(old)
        );
      ...
      if (matches.length === 1) {
        ...
        entry.reason =
          'Symbol still exists in current source but is missing or ambiguous in the new graph';
        ...
        headSource.some(
          (symbol) =>
            symbol.kind === old.kind &&
            symbol.name === old.name &&
            symbol.owner !== null &&
            qualifiedName(symbol.owner).replace(/\s+/g, '') ===
              qualifiedName(old.owner).replace(/\s+/g, '')
        )
      ) {
        entry.reason =
          'Receiver formatting may describe the old identity; type equivalence is not verified';
      } else if (!evidenceValid) {
        entry.reason = 'Strict parser lacks valid scoped symbol evidence; rebuild the core package';
      } else {
        const uncertain = [
          ...compatibleEvidence(baseEvidence.symbolEvidence.coverage.gaps, old).filter(
            (item) => item.reason === 'Unresolved declaration name'
          ),
        ...
        if (uncertain.length) {
          ...
          reusedIndex >= 0 &&
          (!newMappings[reusedIndex] || symbolKey(newMappings[reusedIndex]) !== symbolKey(old))
        ) {
          entry.reason = 'The old graph ID is reused by a different or unresolved source identity';
        } else {
          entry.status = 'deleted';
          entry.reason =
            'Old identity is absent under the declaration coverage profile, with no compatible gaps or effects';
        }
      }
    }
    missing.push(entry);
...
export function git(root, args) {
  ...
  if (result.status !== 0)
    throw new Error(`git ${args[0]} failed: ${result.stderr || result.error || result.status}`);
...
export function loadSymbolContext(projectRoot, intermediateDir) {
  ...
  if (
    ...
    throw new Error('Symbol baseline does not match the incremental plan');
  const paths = baseline.files.map((file) => file.filePath).sort();
  if (
    JSON.stringify(paths) !== JSON.stringify([...plan.filesToReanalyze].sort()) ||
...
export async function validateIncrementalSymbols(projectRoot, { graph, intermediateDir } = {}) {
  ...
  try {
    ...
    if (!Array.isArray(graph.nodes) || !Array.isArray(graph.edges))
      throw new Error('Invalid assembled graph');
    ...
    const parseHead = async (path) => {
      ...
      if (!parser) {
        parser = new core.TreeSitterPlugin(
          core.builtinLanguageConfigs.filter((config) => config.treeSitter)
        );
        await parser.init();
      }
    ...
    for (const previous of baseline.files) {
      ...
      if (
        previous.nodes.some(
          (node) => symbolKind(node) && !hasPreservedIdentity(node, previous, current)
        )
      ) {
        try {
      ...
      if (result.missing.some((node) => node.status !== 'deleted'))
        report.unresolvedFiles.push(previous.filePath);
    }
    ...
    if (report.ok) {
      ...
      if (
        currentCandidates &&
        (currentCandidates.baseCommit !== plan.baseCommit ||
          currentCandidates.headCommit !== plan.headCommit ||
          !Array.isArray(currentCandidates.edges))
      ) {
        throw new Error('Current edge candidates do not match the incremental plan');
      }
      const hasRetry =
        retry?.baseCommit === plan.baseCommit &&
        retry.headCommit === plan.headCommit &&
        Array.isArray(retry.inboundEdgeCandidates);
      if (hasRetry && !Array.isArray(retry.currentFiles))
        throw new Error('Retry endpoint descriptors are missing');
      const candidates = [
      ...
      if (candidates.length || hasRetry) {
        ...
        const replacements = new Map(
          report.files
            .flatMap((file) => file.replacements)
            .map(({ oldId, newId }) => [oldId, newId])
        );
        const deleted = new Set(
          report.files.flatMap((file) => file.missing.map((node) => node.id))
        );
        const baselineBindings = new Map(
          baseline.files
            .flatMap((file) => file.nodes.filter(symbolKind))
            .map((node) => [
              node.id,
              deleted.has(node.id) ? null : (replacements.get(node.id) ?? node.id),
            ])
        );
        ...
        for (const previous of hasRetry ? retry.currentFiles : []) {
          const current = fileGraph(previous.filePath);
          const needsEvidence = previous.nodes.some(
            (node) => symbolKind(node) && !hasPreservedIdentity(node, previous, current, true)
          );
          const evidence =
            needsEvidence && previous.filePath ? await parseHead(previous.filePath) : undefined;
        ...
        const edgeKey = (edge) =>
          JSON.stringify([edge.source, edge.target, edge.type, edge.direction]);
        ...
        for (const { edge: candidate, saved } of candidates) {
          ...
          if (!ids.has(edge.source) || !ids.has(edge.target)) {
            report.droppedCurrentEdges.push({
...
export function formatSymbolReport(report) {
  ...
  for (const file of report.files) {
    lines.push(
      `  ${JSON.stringify(file.filePath)}: nodes ${file.beforeCount} -> ${file.afterCount}; symbols ${file.beforeSymbolCount} -> ${file.afterSymbolCount}`
    );
    for (const node of file.missing)
      lines.push(
        `    ${node.status}: ${JSON.stringify(node.id)} (${JSON.stringify(node.name)}) — ${node.reason}`
      );
  }
  lines.push(...report.errors.map((error) => `  Error: ${error}`));
  if (report.reconciledCurrentEdges)
    lines.push(`  Reconciled ${report.reconciledCurrentEdges} current edge(s)`);
