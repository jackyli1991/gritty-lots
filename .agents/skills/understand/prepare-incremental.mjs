...
function resolveCommit(projectRoot, value) {
  return run('git', ['rev-parse', '--verify', '--end-of-options', `${value}^{commit}`], {
    cwd: projectRoot,
  }).trim();
}
...
function relevantWorktreeChanges(projectRoot, excludePatterns) {
  const paths = sorted([
    ...parseNulPaths(
      run('git', ['diff', '--cached', '--name-only', '-z', '--relative', '--', '.'], {
        cwd: projectRoot,
      })
    ),
    ...parseNulPaths(
      run('git', ['diff', '--name-only', '-z', '--relative', '--', '.'], { cwd: projectRoot })
    ),
    ...parseNulPaths(
...
function inventoryFrom(scan, graph, fingerprints) {
  ...
  return sorted(
    paths.map(normalizeRelativePath).filter((path) => path && !isGeneratedArtifact(path))
  );
}
...
async function main() {
  ...
  const changes = parseNameStatusZ(
    run('git', ['diff', '--name-status', '-z', '--relative', baseCommit, headCommit, '--', '.'], {
      cwd: projectRoot,
    })
  );
  ...
  const baselineScan =
    existingSnapshot?.baseCommit === baseCommit ? existingSnapshot.scan : oldScan;
  const baselineGraph =
    existingSnapshot?.baseCommit === baseCommit && existingSnapshot.graph
  ...
  const trackedPaths = new Set(
    parseNulPaths(run('git', ['ls-files', '--cached', '-z', '--', '.'], { cwd: projectRoot }))
  );
  const currentIgnoreFilter = createIgnoreFilter(projectRoot, args.excludePatterns);
  const unexplainedMissingFiles = oldInventory.filter(
    (path) =>
  ...
  const decision = classifyUpdate(analysis, oldInventory.length, oldInventory, currentInventory);
  ...
  if (!Object.hasOwn(refreshedScan, 'description'))
    refreshedScan.description = 'No description available';
  ...
  const symbolFiles = filesToReanalyze.map((filePath) => {
    const nodes = baselineGraph.nodes.filter(
      (node) => normalizeRelativePath(node.filePath) === filePath
    );
